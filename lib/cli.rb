class CRYPTO::CLI
    ...
    def query_key
        ...
        if @sample_objects[0].has_key?(input)
            CRYPTO::Cryptocurrency.all.each.with_index(1) do |cryptocurrency, index|
                if cryptocurrency.price_change_24h > 0
                    # "$#{cryptocurrency.values_at(input)}"
                    puts ["#{index}.", cryptocurrency.id.capitalize.ljust(20), "$#{cryptocurrency.current_price.to_s.ljust(18)}", "$#{cryptocurrency.roi.to_s.ljust(18)}", cryptocurrency.last_updated.to_s.ljust(18)].join(' ').colorize(:green)
                else
                    puts ["#{index}.", cryptocurrency.id.capitalize.ljust(20), "$#{cryptocurrency.current_price.to_s.ljust(18)}", "$#{cryptocurrency.roi.to_s.ljust(18)}", cryptocurrency.last_updated.to_s.ljust(18)].join(' ').colorize(:red)
                end
            end
        else
